...
const resourcesMenu = navigation.find(
  (item) => item.label === "Resources"
);

export function ResourcesMenu() {
  if (!resourcesMenu?.items) {
    return null;
  }

  const regularItems = resourcesMenu.items.filter(
    (item) => !item.featured
  );

  const featuredItem = resourcesMenu.items.find(
    (item) => item.featured
  );
  ...
  const iconMap = {
    Blog: FileText,
    "SEO Guides": Search,
    "Marketing Guides": Megaphone,
    FAQs: HelpCircle,
    "Free Resources": Sparkles,
  };
      ...
      >
        ...
        >
          ...
          {featuredItem &&
            (() => {
              const FeaturedIcon =
                featuredItem.icon ?? Sparkles;

              return (
                <Link
                  href={featuredItem.href ?? "/resources/free"}
                  className="
                    group/cta
                    mt-4
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted">
                        Discover useful tools, guides and marketing resources.
                      </p>
                    </div>
                  </div>
