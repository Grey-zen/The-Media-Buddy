...
const companyMenu = navigation.find(
  (item) => item.label === "Company"
);

export function CompanyMenu() {
  if (!companyMenu?.items) {
    return null;
  }

  const regularItems = companyMenu.items.filter(
    (item) => !item.featured
  );

  const featuredItem = companyMenu.items.find(
    (item) => item.featured
  );

  const iconMap = {
    "About Us": CircleUserRound,
      ...
      >
        ...
        >
          ...
          {featuredItem &&
            (() => {
              const FeaturedIcon =
                featuredItem.icon ?? Mail;

              return (
                <Link
                  href={featuredItem.href ?? "/contact"}
                  className="
                    group/cta
                    mt-4
